...
cd "$(__get_app_root__)"/"$SHF_BUILD_DIR" &&
git clone "$SHF_REPO_URL" "$SHF_PROJ_NAME_SNAKE" &&
cd "$SHF_PROJ_NAME_SNAKE"  &&
...
if is_ssh; then
	...
	echo 'shf_dev_ops hash:'
	get_hash_of_file './shf_dev_ops.sh'
...
rm -f shf_dev_ops_fifo clone_repo_fifo script_select_fifo
